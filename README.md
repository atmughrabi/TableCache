...
| 31 | `tc_narrow_shim.sv` | **Concurrent reads to consecutive lines that HIT returned the wrong block (the line's *sibling* block, off by one `BLOCK_W`).** The shim issues one wide AR with `arlen=0`, so it expects exactly one R beat. But when two reads to consecutive lines are paired in the `l2_cache` databank's 2-port read pipeline, a hit emits an **extra** R beat: first the requested block (`rlast=1`, correct), then a spurious second beat (`rlast=0`) carrying the line's *other* block. The shim forwarded a narrow response on **every** `m_rvalid` beat, so the trailing beat overwrote the response with the sibling block. Cold misses correctly return one beat (`rlast=1`), so it only surfaced under concurrent hits — reachable by any multi-id master (a protocol-checking `AxiMaster` rejects the extra beat as "unexpected burst ID"), and exposed by the `READ_REORDER_DEPTH>1` reorder buffer (the first thing to drive concurrent same-engine reads). **Fix (shim-side, robust to the databank behavior):** a cache R beat is a real response only when `rlast=1` (`cache_r_resp`); a trailing `rlast=0` beat (`cache_r_extra`) is drained (`m_rready` asserted unconditionally) and **dropped** — never forwarded, and the `rid_outstanding_q`/`cbom_outstanding_q` clears and the line-buffer fill are all gated on `& m_rlast`. The requested block always carries `rlast=1` (upper or lower block of the line), so the requested word is always kept and the sibling always dropped. No-op for the normal single-beat response. Regression: `test_shim_multiread::test_distinct_id_hit_concurrency` (plain multi-id master, concurrent hits, both blocks, DB_LATENCY 1/2) + `test_shim_reorder::test_reorder_out_of_order_completion` (miss at head, warm hits behind it → out-of-order cache completion). |
