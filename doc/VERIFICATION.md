...
| `victim_cache.sv` | **100%** | 4 | 0 | `negate_hit` + `drop_buffer_tag_uncacheable` + `drop_invalidate_clear` + `swap_write_hit_check`. Required two non-obvious test design insights: (1) victim's line_storage only receives data on DIRTY eviction (clean evicts buffer only the tag); (2) detecting swap_write_hit_check requires multi-entry observation -- the matching tag survives in both branches, but OTHER tags disappear under mutation. Test `test_victim_write_hit_preserves_others` uses 4 lines in different sets to make the side effect visible |
