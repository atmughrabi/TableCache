...
| `victim_cache.sv` | **75%** | 3 | 1 | `negate_hit` + `drop_buffer_tag_uncacheable` + `drop_invalidate_clear` killed; `swap_write_hit_check` survives. The fix for `drop_invalidate_clear` was to dirty the line BEFORE evicting it (so victim's line_storage actually receives the dirty data) -- otherwise victim only buffers a tag-only entry with no data to return, masking the mutation. The remaining survivor requires multi-entry observation: under mutation, write_hit_one_hot fires on non-matching tags so OTHER victim entries get invalidated while the matching one survives with stale data. A directed test that observes those other entries' survival would close the gap |
...
| `one_hot_to_integer.sv` | **100%** | 1 | 0 | `drop_or_combine` killed by every test that uses replacement_policy |
| `l2_top.sv` | n/a | 0 | 0 | Pure AXI-port adapter not instantiated by any cocotb test; documented out-of-test-scope |
