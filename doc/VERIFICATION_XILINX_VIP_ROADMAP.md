...
| A18 | post-PnR functional sim against the routed netlist | 2 | 4 | **SCAFFOLDED** in 2026-05. `syn/vivado/v80_synth_pnr.tcl` now emits `routed_netlist.v` + `routed_netlist.sdf` after route + phys_opt. `syn/post_pnr_sim.sh` locates the netlist and documents the remaining wiring (cocotb XSIM_BIN env var + unisims_ver compile + SDF back-annotation). Verilator is not a viable backend (UNISIM primitives + glbl). Full xsim hookup is ~1 day and shares infrastructure with D1 (Xilinx AXI VIP). |
